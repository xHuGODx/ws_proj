...
\begin{titlepage}
  ...
  \begin{tabular}{rl}
    \textbf{Author}      & Rodrigo Abreu (113626), Hugo Ribeiro () \\[0.3em]
...
(\url{https://www.kaggle.com/datasets/rohanrao/formula-1-world-championship-1950-2020?select=circuits.csv}), a free API and database export providing
structured Formula 1 statistics from 1950 to the present. For this project the
data was downloaded as a CSV export and covers the \textbf{1950--2024} seasons.
...
\subsubsection{LLM Assistant}

The public interface now includes an \texttt{/assistant/} page where users can
ask natural-language questions about the Formula 1 knowledge graph. The server
uses a Gemini model with a hardcoded schema/context prompt describing the main
classes, predicates, and identifier conventions of the RDF graph. The workflow
is two-stage:
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item Integrate live data from the current season via the Ergast API.
  \item Replace the hardcoded assistant schema prompt with a richer ontology-aware
        context builder generated from the graph itself.
\end{itemize}
...
\begin{enumerate}[leftmargin=2em]
  \item Place the Ergast CSV files in \texttt{data/raw/}.
